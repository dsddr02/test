...
class DataFetcher:
    ...
    def fetch(self):
        ...
        logging.info(f"Here are a total of {len(user_id_list)} userids, which are among which {self.IGNORE_USER_ID} will be ignored.")
        time.sleep(self.RETRY_WAIT_TIME_OFFSET_UNIT)
        ...
        for userid_index, user_id in enumerate(user_id_list):           
            try: 
                ...
                if current_userid in self.IGNORE_USER_ID:
                    logging.info(f"The user will be ignored in user_id_list")
                    continue
                else:
                    ### get data 
    ...
    def _get_all_data(self, driver, user_id, userid_index):
        ...
        if yearly_usage is None:
            logging.error(f"Get year power usage for user failed, pass")
        else:
            logging.info(
                f"Get year power usage for user successfully, usage is {yearly_usage} kwh")
        if yearly_charge is None:
            logging.error(f"Get year power charge for user failed, pass")
        else:
            logging.info(
                f"Get year power charge for user successfully, yealrly charge is {yearly_charge} CNY")

        # 按月获取数据
        month, month_usage, month_charge = self._get_month_usage(driver)
        if month is None:
            logging.error(f"Get month power usage for user failed, pass")
        else:
            for m in range(len(month)):
                logging.info(f"Get month power charge for user successfully, {month[m]} usage is {month_usage[m]} KWh, charge is {month_charge[m]} CNY.")
        # get yesterday usage
        last_daily_date, last_daily_usage = self._get_yesterday_usage(driver)
        if last_daily_usage is None:
            logging.error(f"Get daily power consumption for user failed, pass")
        else:
            logging.info(
                f"Get daily power consumption for user successfully, , {last_daily_date} usage is {last_daily_usage} kwh.")
        if month is None:
            logging.error(f"Get month power usage for user failed, pass")
